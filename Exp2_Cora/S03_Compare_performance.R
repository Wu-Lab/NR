...
g_origi <-
  graph_from_adjacency_matrix(adj_origi, mode = 'undirected', weighted = T)
...
g_NR_once <-
  graph_from_adjacency_matrix(out_NR, mode = 'undirected', weighted = T)
...
g_NR_twice <-
  graph_from_adjacency_matrix(out_NR_twice,  mode = 'undirected', weighted = T)
...
g_katz_once <-
  graph_from_adjacency_matrix(out_katz, mode = 'undirected', weighted = T)
...
g_katz_twice <-
  graph_from_adjacency_matrix(out_katz_twice, mode = 'undirected', weighted = T)
...
g_LP_once <-
  graph_from_adjacency_matrix(out_LP, mode = 'undirected', weighted = T)
...
g_LP_twice <-
  graph_from_adjacency_matrix(out_LP_twice, mode = 'undirected', weighted = T)
...
g_CN_once <-
  graph_from_adjacency_matrix(out_CN, mode = 'undirected', weighted = T)
...
g_CN_twice <-
  graph_from_adjacency_matrix(out_CN_twice, mode = 'undirected', weighted = T)
...
g_NE_once <-
  graph_from_adjacency_matrix(out_NE_once, mode = 'undirected', weighted = T)
...
out_NE_twice <-
  as.matrix(read.csv(
    "./Exp3_Cora/Data/Network_NE2.csv",
    sep = ',',
    encoding = "UTF-8",
    header = F
  ))
...
g_NE_twice <-
  graph_from_adjacency_matrix(out_NE_twice, mode = 'undirected', weighted = T)
...
score_NMI <-
  matrix(0, nrow = length(Methods), ncol = length(col_content))
...
write.csv(score_NMI, './Exp3_Cora/Results/score_NMI.csv')
write.csv(score_ARI, './Exp3_Cora/Results/score_ARI.csv')
